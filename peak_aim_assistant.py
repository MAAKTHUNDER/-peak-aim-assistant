...
class SettingsDialog(QDialog):
    def __init__(self, parent, current_hotkey, current_aim,
        ...
        # Voice Control
        voice_title = QLabel("🎤 Voice Control (Vosk - Offline):")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Peak & Aim Assistant v1.0 (Vosk)")
        ...
        footer = QLabel("Made for GameLoop | v1.0 (Vosk)")
    ...
    def setup_tray(self):
        ...
        self.tray_icon.setToolTip("Peak & Aim Assistant - MAAKTHUNDER (Vosk)")
        icon_path = resource_path("icon.ico")
        if os.path.exists(icon_path):
...
def main():
    ...
    shared_mem = QSharedMemory("PeakAimAssistantVoskUniqueName")
    if not shared_mem.create(1):
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle("Already Running")
        msg.setText("Peak & Aim Assistant (Vosk) is already running!")
