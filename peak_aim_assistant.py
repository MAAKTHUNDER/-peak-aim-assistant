            ...
            try:
                ...
                with self.microphone as source:
                    try:
                        ...
                        continue
                try:
                    # Add spaces around text to match commands with spaces
                    text = ' ' + self.recognizer.recognize_google(audio).lower().strip() + ' '
                    self.status_update.emit(f"Heard: {text.strip()}")
                    
                    matched = False
                    
                    # Check TOGGLE first (highest priority)
                    ...
                    # Then check OFF (before ON to avoid "turn on" matching "on")
                    if not matched:
...
class MainWindow(QMainWindow):
    ...
    def on_voice_command(self, command):
        ...
        elif command == 'on' and not self.macro_thread.enabled:
